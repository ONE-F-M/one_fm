...
@frappe.whitelist()
def get_unassigned_project_employees(project, start_date, end_date, limit_start=None, limit_page_length=20):
	#Todo add date range
...
@frappe.whitelist()
def get_assigned_employees(shift, start_date, end_date, limit_start=None, limit_page_length=20):
	#Todo add date range
	return frappe.get_list("Roster", fields=["employee", "employee_name", "post_type"], filters={"shift": shift}, order_by="employee_name asc",
		limit_start=limit_start, limit_page_length=limit_page_length, ignore_permissions=True)
