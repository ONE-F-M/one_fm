...
def notify_approver_about_pending_shift_request(is_scheduled_event=True):
	...
	if pending_shift_request:
		...
		for obj in pending_shift_request:
			if obj["custom_reports_to_user"]:
				if not data_dict.get(obj["custom_reports_to_user"]):
					data_dict.update({obj["custom_reports_to_user"]: list()})
			elif obj["custom_project_manager_user"]:
				if not data_dict.get(obj["custom_project_manager_user"]):
					data_dict.update({obj["custom_project_manager_user"]: list()})

		for obj in pending_shift_request:
			if obj["custom_reports_to_user"]:
				data_dict.get(obj["custom_reports_to_user"]).append({obj.get("employee_name"): get_url_to_form("Shift Request", obj.get("name"))})
			elif obj["custom_project_manager_user"]:
				data_dict.get(obj["custom_project_manager_user"]).append({obj.get("employee_name"): get_url_to_form("Shift Request", obj.get("name"))})
