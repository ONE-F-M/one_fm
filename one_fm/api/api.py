...
def test_push_notification():
    ...
    body = "Final Reminder: Please checkout in the next five minutes."
    """
    {    
    "to":"dSfg-EzKSZGpXHYo-HoMzq:APA91bF-ldjIx7ePhfyrFOa-7csn5U1yIiAtOdWl_zgEXvX38A7TFRzP4KqxI0VHsM2YaE1UbBBvjyyxlvjKvGBNm-MYSSD_yaHQHdpHyXoczvKTaWW9flIS5jp2UujqjEjiz3LoorZz",
    "data": {
    ...
    """
    registration_token = frappe.get_all("Employee", {"name": employee}, "fcm_token")
    message = messaging.Message(
            notification=messaging.Notification(title, body),
            android=messaging.AndroidConfig(
                notification=messaging.AndroidNotification(
                    clickAction= "oneFmNotificationCategory1"
                ),
            ),
            token=registration_token[0].fcm_token,
            )
    response = messaging.send(message)
