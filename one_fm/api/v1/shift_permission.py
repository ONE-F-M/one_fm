...
def approve_shift_permission(employee_id: str = None, shift_permission_id: str = None):
    ...
    try:
        ...
        if shift_permission_doc.workflow_state == "Pending":
            ...
            subject = _("{name} has approved the permission on {date}.".format(name=supervisor_name, date=shift_permission_doc.date))
            message = _("{name} has approved the permission on {date}.".format(name=supervisor_name, date=shift_permission_doc.date))
...
def reject_shift_permission(employee_id: str = None, shift_permission_id: str = None):
    ...
    try:
        ...
        if shift_permission_doc.workflow_state == "Pending":
            ...
            user_id, supervisor_name= frappe.db.get_value('Employee',{'name':shift_supervisor},['user_id','employee_name'])
            subject = _("{name} has rejected the permission on {date}.".format(name=supervisor_name, date=shift_permission_doc.date))
            message = _("{name} has rejected the permission on {date}.".format(name=supervisor_name, date=shift_permission_doc.date))
            notify_for_shift_permission_status(subject, message,user_id, shift_permission_doc, 1)
