...
class TestCompanyCreationWarehouseType(FrappeTestCase):
	...
	def test_company_creation_succeeds_after_seeding_transit_warehouse_type(self):
		"""WI-000446 fix: once create_warehouse_type_transit() has run (as it does from
		the patch and from after_install()), creating a Company succeeds and a
		"Goods In Transit" Warehouse with warehouse_type="Transit" exists for it."""
		_delete_transit_warehouse_type()
		...
		transit_warehouse = frappe.db.get_value(
			"Warehouse",
			{"company": TEST_COMPANY, "warehouse_type": "Transit"},
			"name",
		)
		self.assertTrue(transit_warehouse)
