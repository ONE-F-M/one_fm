<template>
  <div class="flex flex-col">
    ...
    <div class="p-5 w-full overflow-y-scroll">
      ...
      >
        <!-- Trend + Master Data Charts in a single flowing grid so an odd
             number of charts only leaves a trailing gap at the very bottom
             (one_fm override adds a "Tickets by Status" trend chart) -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-full mt-4">
          <template
            v-for="(chart, index) in [
              ...(trendData.data || []),
              ...(masterData.data || []),
            ]"
            :key="index"
