...
// Visa Costing carries a flat 20 KWD visa fee.
const VISA_COSTING_ACTION = 'Visa Costing';
const VISA_COSTING_AMOUNT = 20;

frappe.ui.form.on('GRD Renewal Extension Cost', {
	renewal_or_extend: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		var amount = child.visa_amount;

		if(child.renewal_or_extend === VISA_COSTING_ACTION){
			// Fill an empty amount only, so a rate set to something else is kept.
			// set_value fires the visa_amount handler below, which totals the row.
			if(!amount){
				frappe.model.set_value(cdt, cdn, 'visa_amount', VISA_COSTING_AMOUNT);
			}
		} else if(amount === VISA_COSTING_AMOUNT){
			...
			frappe.model.set_value(cdt, cdn, 'visa_amount', 0);
		}
	},
	visa_amount: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
...
var caclulate_renewal_extension_cost_total = function(frm, child) {
	...
	if(child.visa_amount && child.visa_amount > 0){
		total_cost += child.visa_amount;
	}
