...
class CreateMap():
    ...
    def __init__(self,start,end,employees,filters,isOt):
        ...
        if len(employees)==1:
            ...
            self.attendance_query = f"SELECT at.status,at.leave_application, at.attendance_date,at.employee,at.employee_name from `tabAttendance`at  where at.employee in {self.employees}  and at.attendance_date between '{self.start}' and '{add_to_date(cstr(getdate()), days=-1)}' order by at.employee """
