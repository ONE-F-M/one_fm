...
class CreateMap():
    ...
    def __init__(self,start,end,employees,filters,isOt):
        ...
        if len(employees)==1:
            ...
            self.employee_query = f"SELECT name,relieving_date, employee_name,day_off_category,number_of_days_off from `tabEmployee` where name in ('{employees[0].employee}') order by employee_name"
        else:
            self.schedule_query  = f"SELECT  es.employee, es.employee_name, es.date, es.operations_role, es.post_abbrv, \
                es.shift, es.roster_type, es.employee_availability, es.day_off_ot, es.project from `tabEmployee Schedule`es  where \
            ...
            self.employee_query = f"SELECT name, relieving_date, employee_name,day_off_category,number_of_days_off from `tabEmployee` where name in {self.employees} order by employee_name"
