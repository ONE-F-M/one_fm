...
function renderManifest($container, data) {
	...
	window._mfst_markAllPresent = function (button) {
		let rowIds;
		try {
			rowIds = JSON.parse($(button).attr("data-rows") || "[]");
		} catch (e) {
			rowIds = [];
		}
		if (!rowIds.length) return;

		// Disabled immediately: a driver double-tapping at a departure should not send the
		// same stop twice.
		$(button).prop("disabled", true);

		frappe.call({
			method: "one_fm.one_fm.api.doc_methods.transportation_manifest.mark_manifest_rows_present",
			args: { row_names: rowIds },
			freeze: true,
			freeze_message: __("Marking everyone present…"),
			callback: function (r) {
				if (!r.message || !r.message.rows) return;

				r.message.rows.forEach(function (row) {
					window.checkerState[row.name] = window.checkerState[row.name] || {};
					window.checkerState[row.name].attendance = row.attendance_status;
					window.checkerState[row.name].qoa = row.qoa_status;
					window.checkerState[row.name].qoa_fail_reason = row.qoa_reason;
				});

				// AC2: every chip goes solid green with its checkmark, which is what
				// empChipHtml already draws for Present + Pass.
				if (activeView) renderRoute(activeView);
			},
			always: function () {
	...
	function uncheckedRowIds(employees) {
		return (employees || []).map(function (e) {
			...
			return (typeof e === "object" && e !== null) ? (e.row_id || id) : id;
		}).filter(function (rowId) {
			return rowId && !(window.checkerState[rowId] || {}).attendance;
		});
	}

	function bulkPresentBtnHtml(employees) {
		const pending = uncheckedRowIds(employees);
		if (!pending.length) {
			return "";
		}

		// The ids ride on a data attribute rather than in the onclick string: a row name is
		// generated by Frappe and needs no quoting here, and the handler reads them back
		// with JSON.parse so one malformed id cannot break the whole card.
		const payload = JSON.stringify(pending).replace(/&/g, "&amp;").replace(/"/g, "&quot;");
		return `<button class="mfst-depart-btn mfst-bulk-present" data-rows="${payload}"
			onclick="window._mfst_markAllPresent(this)">
			<span class="material-symbols-outlined" style="font-size:16px;vertical-align:middle">done_all</span>
			Mark All Present (${pending.length})</button>`;
	}
	...
	function renderDepartCard(time, camp, activeStop, manifestName, vehicleLabel, isMixed,
		...
		if (employees.length > 0) {
			...
			// AC1/AC2: a bulk action beside the tap hint, on an active stop only, and only
			// while somebody is still unchecked. A departure is not the moment to open
			// twenty modals one at a time.
			const bulkBtn = isActive ? bulkPresentBtnHtml(employees) : "";
...
function getManifestCSS() {
	return `
		...
		.mfst-depart-btn.mfst-bulk-present:disabled { opacity: 0.6; cursor: default; }
