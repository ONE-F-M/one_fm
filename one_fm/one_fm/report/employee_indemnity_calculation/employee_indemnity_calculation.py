...
def get_columns(filters):
	...
	columns = [
		...
		_("Total Period Till Date")+ ":Float:160"
	]
...
def get_data(filters):
	...
	for employee in active_employees:
		indemnity_allcn = frappe.get_doc('Indemnity Allocation', {'employee': employee.name, 'expired': False})
		if (user in ["Administrator", employee.user_id]) or ("HR Manager" in frappe.get_roles(user)):
			...
			indemnity_as_on = indemnity_allcn.total_indemnity_allocated
			amount_as_on = indemnity_as_on * salary_per_day
			provision_indemnity = (30/365)*30
			total_period_till_date = 0
			...
			row += [indemnity_as_on, amount_as_on, provision_indemnity, provision_amount, total_indemnity, total_amount, total_period_till_date]
