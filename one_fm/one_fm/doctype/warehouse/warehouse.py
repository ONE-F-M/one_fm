...
class Warehouse(NestedSet):
    ...
    def autoname(self):
        self.name = self.warehouse_code+' - '+self.warehouse_name
    ...
    def validate_get_parent(self):
        new_warehouse_code = frappe.db.sql("select warehouse_code+1 from `tabWarehouse` where parent ='{0}' order by warehouse_code desc limit 1".format(self.parent_warehouse))
        if new_warehouse_code:
            new_warehouse_code_final = new_warehouse_code[0][0]
        else:
            new_warehouse_code_final = '1'

        self.warehouse_code = str(int(new_warehouse_code_final)).zfill(3)
