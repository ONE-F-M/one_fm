...
class RequestforQuotation(BuyingController):
    ...
    def get_supplier_group_list(self,supplier_group):
        supplier_group_list = frappe.db.sql("select parent from `tabSupplier Group Table` where parenttype='Supplier' and supplier_group='{0}' order by parent".format(supplier_group),as_list=True)
    ...
    def send_supplier_quotation_emails(self):
        for supplier in self.suppliers:
            msg = frappe.render_template('one_fm/templates/emails/request_for_quotation.html', context={"items": self.items,"message_for_supplier": self.message_for_supplier,"terms": self.terms})
