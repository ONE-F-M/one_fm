...
function load_recruitment_plan_autocomplete_options(frm) {
	...
	if (frappe[COUNTRY_KEY] && frappe[NATIONALITY_KEY]) {
		_populate_autocomplete(frm, "trip_country", frappe[COUNTRY_KEY]);
		_populate_autocomplete(frm, "nationality_in_pr_request", frappe[NATIONALITY_KEY]);
		return;
	}
	...
	frappe.call({
		...
		callback: function(r) {
			if (r.message) {
				...
				_populate_autocomplete(frm, "trip_country", countries);
				...
				_populate_autocomplete(frm, "nationality_in_pr_request", nationalities);
			}
		}
	});
}
