...
def get_data(filters):
	...
	# 			WHERE sale_item="{i.item_code}" AND project="{i.project}"
	# 		""", as_dict=1)]
	# 		post_schedule = frappe.db.get_list("Post Schedule", filters={
	# 			'project':i.project, 'operations_role': ['IN', roles], 'date': ['BETWEEN', [getdate().replace(day=1), to_date]]})
	# 		if post_schedule:
	# 			i.projection = len(post_schedule)/i.count
