...
class OperationsTask(NestedSet):
	...
	def validate_parent_project_dates(self):
		...
		expected_end_date = frappe.db.get_value("Operations Project", self.project, "expected_end_date")
	...
	def validate_status(self):
		if self.status!=self.get_db_value("status") and self.status == "Completed":
			for d in self.depends_on:
				if frappe.db.get_value("Operations Task", d.task, "status") not in ("Completed", "Cancelled"):
	...
	def update_project(self):
		if self.project and not self.flags.from_project:
			frappe.get_cached_doc("Operations Project", self.project).update_project()
	...
	def reschedule_dependent_tasks(self):
		...
		if end_date:
			for task_name in frappe.db.sql("""
				select name from `tabOperations Task` as parent
				where parent.project = %(project)s
					and parent.name in (
						select parent from `tabTask Depends On` as child
						where child.task = %(task)s and child.project = %(project)s)
			""", {'project': self.project, 'task':self.name }, as_dict=1):
				task = frappe.get_doc("Operations Task", task_name.name)
	...
	def populate_depends_on(self):
		if self.parent_task:
			parent = frappe.get_doc('Operations Task', self.parent_task)
...
def check_if_child_exists(name):
	child_tasks = frappe.get_all("Operations Task", filters={"parent_task": name})
	child_tasks = [get_link_to_form("Operations Task", task.name) for task in child_tasks]
	return child_tasks


def get_project(doctype, txt, searchfield, start, page_len, filters):
	from erpnext.controllers.queries import get_match_cond
	return frappe.db.sql(""" select name from `tabOperations Project`
...
def set_multiple_status(names, status):
	...
	for name in names:
		task = frappe.get_doc("Operations Task", name)
		task.status = status
		task.save()

def set_tasks_as_overdue():
	tasks = frappe.get_all("Operations Task", filters={"status": ["not in", ["Cancelled", "Completed"]]}, fields=["name", "status", "review_date"])
	for task in tasks:
		if task.status == "Pending Review":
			if getdate(task.review_date) > getdate(today()):
				continue
		frappe.get_doc("Operations Task", task.name).update_status()
...
def add_multiple_tasks(data, parent):
	...
	new_doc = {'doctype': 'Operations Task', 'parent_task': parent if parent!="All Tasks" else ""}
	new_doc['project'] = frappe.db.get_value('Operations Task', {"name": parent}, 'project') or ""
...
def on_doctype_update():
	frappe.db.add_index("Operations Task", ["lft", "rgt"])
