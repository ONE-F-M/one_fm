frappe.listview_settings['Operations Task'] = {
	add_fields: ["project", "status", "priority", "exp_start_date",
		"exp_end_date", "subject", "progress", "depends_on_tasks"],
	filters: [["status", "=", "Open"]],
	onload: function(listview) {
		var method = "one_fm.operations.doctype.operations_task.operations_task.set_multiple_status";
	...
	gantt_custom_popup_html: function(ganttobj, task) {
		var html = `<h5><a style="text-decoration:underline"\
			href="#Form/Operations Task/${ganttobj.id}""> ${ganttobj.name} </a></h5>`;
