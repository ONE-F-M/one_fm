			...
			frm.add_custom_button(
				...
				() => {
					let post_dialog = new frappe.ui.Dialog({
						'fields': [
							{'label': 'Number of Posts', 'fieldname': 'qty', 'fieldtype': 'Int' ,onchange: function(){
								// let post_number = this.value;
								// if(post_number !== undefined){
								// 	// console.log(post_dialog.fields_dict["post_names"].grid.grid_rows);
								// 	post_dialog.fields_dict["post_names"].grid.grid_rows.forEach(row => {
								// 		console.log(row);
								// 		row.remove();
								// 	});
								// 	for(let i=0; i<post_number; i++){
								// 		post_dialog.fields_dict["post_names"].grid.df.data.push({idx: 1, __islocal: true});
								// 	}
								// 	post_dialog.fields_dict["post_names"].grid.refresh();
								// }
							}},
							{
								'label': 'Post Names', 
