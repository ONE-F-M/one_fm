...
class OperationsProject(Document):
	def get_feed(self):
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			frappe.db.set_value('Operations Project', new_name, 'copied_from', new_name)
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	return frappe.db.sql('''select distinct project.*
		from `tabOperations Project` project, `tabProject User` project_user
		where
		...
		'''.format(limit_start, limit_page_length),
						 ...
						 as_dict=True,
						 update={'doctype': 'Operations Project'})
...
def get_cost_center_name(project):
	return frappe.db.get_value("Operations Project", project, "cost_center")
...
def send_project_status_email_to_users():
	...
	for d in frappe.get_all("Project Update",
		...
		project_doc = frappe.get_doc('Operations Project', doc.project)
...
def update_project_sales_billing():
	...
	exists_query = '(SELECT 1 from `tab{doctype}` where docstatus = 1 and project = `tabOperations Project`.name)'
	project_map = {}
	for project_details in frappe.db.sql('''
			SELECT name, 1 as order_exists, null as invoice_exists from `tabOperations Project` where
			exists {order_exists}
			union
			SELECT name, null as order_exists, 1 as invoice_exists from `tabOperations Project` where
		...
		project = project_map.setdefault(project_details.name, frappe.get_doc('Operations Project', project_details.name))
...
def set_project_status(project, status):
	...
	project = frappe.get_doc('Operations Project', project)
