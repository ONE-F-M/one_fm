...
def create_sales_invoice():
    ...
    for contract in active_contracts:
        ...
        if not contract_doc.items:
            frappe.throw(_("No contract items found for contract: {contract}".format(contract=contract)))
        
        billing_type = str(contract_doc.billing_type)

        if billing_type.lower() == "hourly":
            service_items_hourly_amounts = get_service_items_hourly_invoice_amounts(contract_doc)
            print(service_items_hourly_amounts)

        elif billing_type.lower() == "daily":
            print("Daily billing type invoice calculation")
...
def get_service_items_hourly_invoice_amounts(contract):
    ...
    for item in contract.items:
        ...
        if item_group.lower() == "service":
            ...
            elif contract.monthly_working_days > 0:
                days = cint(contract.monthly_working_days)

            else:
                frappe.throw(_("No working days set for item {item_code}/ No monthly working days set for contract {contract}".format(item_code=item_code, contract=contract.name)))
            
            if cint(item.head_count) == 0:
                frappe.throw(_("Head count cannot be zero for item: {item_code}".format(item_code=item.item_code)))

            shift_hours = item.shift_hours

            item_hours = 0
            expected_item_hours = days * shift_hours * cint(item.head_count)
            amount = 0
