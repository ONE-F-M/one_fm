...
def queue_send_workflow_action_email(doc, recipients):
    ...
    response= get_mandatory_fields(doc.doctype, doc.name)
    ...
    if not list(user_data_map.values()):
        email_args = {
            ...
            "args": {
                ...
                "mandatory_field": response.get('mandatory_field'),
                "field_labels": response.get('labels')
            },
...
def get_mandatory_fields(doctype, doc_name):
    ...
    response = get_doctype_mandatory_fields(doctype)
    
    doc = frappe.get_value(doctype, {'name':doc_name}, response.get('mandatory_fields'), as_dict=True)
    if doc:
        for employee_field in response.get('employee_fields'):
        ...
        return {'doc':doc, 'labels':response.get('labels')}
...
def get_doctype_mandatory_fields(doctype):
    ...
    return {'mandatory_fields':mandatory_fields,'employee_fields':employee_fields, 'labels':labels}
...
def get_assignment_rule_description(doctype):
    ...
    if response.get('mandatory_fields'):
        message_html += '''
            <br/>
            ...
            <table cellpadding="0" cellspacing="0" border="1" style="border-collapse: collapse;">
                ...
                <tbody>
        '''
        for mandatory_field in response.get('mandatory_fields'):
            message_html += '''
                <tr>
                    <td style="padding: 10px;">'''+response.get('labels')[mandatory_field]+'''</td>
