...
class TestTheDocTypeMatchesTheBASite(FrappeTestCase):
	...
	def test_the_ba_field_order_is_kept(self):
		"""Only the reason WI-002428 adds is inserted; everything else is in their order."""
...
class TestOneLiveCancellationPerVisa(FrappeTestCase):
	...
	def test_a_rejected_one_frees_the_visa(self):
		...
		if not has_workflow_state_column():
			self.skipTest("no workflow on Visa Cancellation Request on this site yet")
	...
	def test_the_escape_hatch_is_in_the_query(self):
		"""The branch the test above has to skip until the process map brings its workflow.
		Asked of the filters rather than of the database, so the rule the escape hatch
		depends on is checked on a site that has not got there yet."""
		filters = live_cancellation_filters(self.visa, with_state=True)

		self.assertIn(["workflow_state", "!=", REJECTED_STATE], filters)

	def test_without_the_column_the_rest_of_the_rule_still_holds(self):
		"""A site with no workflow yet still gets one cancellation per visa - it simply
		has no state to make an exception for."""
		filters = live_cancellation_filters(self.visa, with_state=False)

		self.assertNotIn(["workflow_state", "!=", REJECTED_STATE], filters)
	...
	def test_an_unnamed_document_does_not_switch_the_rule_off(self):
		...
		duplicate."""
		filters = live_cancellation_filters(self.visa, exclude=None, with_state=False)
