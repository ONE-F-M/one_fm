...
def live_cancellation_filters(visa_request: str, exclude=None, with_state=True) -> list:
	"""What counts as a cancellation still standing against this Visa Request (WI-002432).

	A cancelled document does not stand - it has been withdrawn - and neither does one the
	process map has put into "Visa Cancellation Rejected", which is the story's own escape
	hatch: the PRO Operator refused this attempt, so another may be raised.

	`with_state` is false only where the site has no workflow_state column yet, which is
	until the Visa Cancellation workflow arrives with the process map. Split out from the
	query so the escape hatch can be checked on a site that has not got there.
	"""
	filters = [
	...
	if with_state:
		filters.append(["workflow_state", "!=", REJECTED_STATE])
...
def live_cancellation(visa_request: str, exclude: str | None = None) -> str | None:
	...
	return frappe.db.get_value(
		...
		live_cancellation_filters(visa_request, exclude, with_state=has_workflow_state_column()),
		"name",
	)
