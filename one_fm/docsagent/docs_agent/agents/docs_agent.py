...
class DocsAgent:
        ...
        if init_chroma:
            for item in self.config.db_configs:
                if "chroma" in item.db_type:
                    item.vector_db_dir =resolve_path(item.vector_db_dir)
    ...
    def ask_aqa_model_using_local_vector_store(
        ...
        if self.config.log_level == "VERBOSE":
            ...
            self.print_the_prompt(verbose_prompt)
        try:
            final_context = aqa_response.answer.grounding_attributions[0].content.parts[0].text
            return aqa_response.answer.content.parts[0].text, final_context
        except:
            self.aqa_response_buffer = ""
