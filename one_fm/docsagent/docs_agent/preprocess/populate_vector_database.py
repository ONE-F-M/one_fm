...
from one_fm.docsagent.docs_agent.storage.google_semantic_retriever import SemanticRetriever
from one_fm.docsagent.docs_agent.utilities import config
from one_fm.docsagent.docs_agent.utilities.config import ConfigFile
from one_fm.docsagent.docs_agent.utilities.config import ProductConfig
from one_fm.docsagent.docs_agent.utilities.helpers import end_path_backslash
...
def process_all_products(
    ...
    for product in config_file.products:
        ...
        print(f"Output directory: {resolve_path(product.output_path)}")
        print(f"Database operation db type: {product.db_type}")
        print()
        product.output_path =resolve_path(product.output_path)
