...
# The HR Costing table gets a "Visa Costing" Action whose Visa Amount is a flat 20 KWD.
#
...
def verify():
	...
	row = frappe.db.get_value(
		...
		["visa_amount", "total_amount"],
		as_dict=True,
	)
	if not row:
		frappe.throw(f"{ACTION} was not added to the {PARENT} costing table")
	if row.visa_amount != VISA_AMOUNT:
		frappe.throw(
			f"{ACTION} was added with a visa amount of {row.visa_amount}, not {VISA_AMOUNT}"
		)
