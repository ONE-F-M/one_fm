...
function cleanupOS(os, pattern, label) {
  ...
  return format((_a = os.replace(/ ce$/i, " CE").replace(/\bhpw/i, "web").replace(/\bMacintosh\b/, "Mac OS").replace(/_powerpc\b/i, " OS").replace(/\b(os x) [^\d ]+/i, "$1").replace(/\bMac (OS X)\b/, "$1").replace(/\/(\d)/, " $1").replace(/_/g, ".").replace(/(?: bepc|[ .]*fc[\d .]+)$/i, "").replace(/\bx86\.64\b/gi, "x86_64").replace(/\b(Windows Phone) OS\b/, "$1").replace(/\b(Chrome OS \w+) [\d.]+\b/, "$1").split(" on ")[0]) != null ? _a : "");
}
...
function range(start, end) {
  ...
  if (!isNumber(end)) {
    return Array.from({ length: Math.abs(startValue) }, (_, index) => (startValue < 0 ? -1 : 1) * index);
  }
  if (startValue <= end) {
...
function cx(...classes) {
  return uniqueArray((0, import_core.cx)(...classes).split(" ")).join(" ");
}
...
//# sourceMappingURL=remirror-core-helpers.cjs.map
